...
\textbf{Вкладка 2 --- Сортировка.} Позволяет выбрать входной файл (через диалог), выходной файл, столбец сортировки (Combobox из 25 имён колонок), порядок (возрастание/убывание) и язык (Python / C++). При выборе C++ выполняется компиляция через \texttt{g++~-std=c++17~-O2} и запуск через \texttt{subprocess.run}. Временной экзешник автоматически удаляется после завершения.
