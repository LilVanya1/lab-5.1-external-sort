...
# Ключи CSV (generator) + подписи в GUI (пивные магазины)
COLUMN_DEFS = [
    ("id", "ID"),
    ("surname", "Название"),
    ("faculty", "Район"),
    ("course", "Категория"),
    ("favorite_drink", "Хит продаж"),
    ("alcohol_budget", "Бюджет"),
    ("stipendia", "Закупка"),
    ("percent_on_alcohol", "Маржа %"),
    ("drinking_place", "Расположение"),
    ("morning_hangover", "Жалобы"),
    ("skipped_classes", "Дефицит"),
    ("favorite_excuse", "Причина"),
    ("slept_in_bushes", "Без лицензии"),
    ("lost_item", "Потери"),
    ("expulsion_status", "Лицензия"),
    ("drunk_fights", "Инциденты"),
    ("drunk_love_confessions", "Отзывы"),
    ("drunk_calls_to_ex", "Поставщик"),
    ("max_promille", "Крепость"),
    ("nickname", "Вывеска"),
    ("date", "Дата открытия"),
    ("avg_grade", "Рейтинг"),
    ("age", "Клиенты ежедневно"),
    ("debt", "Долг"),
]

COL_IDS = [c[0] for c in COLUMN_DEFS]
COL_LABELS = [c[1] for c in COLUMN_DEFS]
COL_WIDTHS = [
    50, 140, 100, 50, 150, 80, 80, 80, 150, 80,
    80, 150, 80, 120, 150, 60, 80, 60, 80, 120, 90, 60, 50, 50,
]


def _normalize_csv_row(row: list[str]) -> list[str]:
    """Старые CSV (25 полей с name) → 24 поля без отдельного name."""
    if len(row) >= 25:
        merged = f"{row[1]} {row[2]}".strip() if row[2] else row[1]
        row = [row[0], merged] + row[3:25]
    n = len(COL_IDS)
    while len(row) < n:
        row.append("")
    return row[:n]
...
class App:
    def __init__(self):
        self.root = tk.Tk()
        self.root.title("Магазины — Сортировка")
        self.root.geometry("950x650")
        self.col_names = COL_LABELS
    ...
    def build_gen_tab(self, parent):
        ...
        self.gen_file = tk.StringVar(value="magazines.csv")
    ...
    def build_sort_tab(self, parent):
        ...
        self.sort_in = tk.StringVar(value="magazines.csv")
        ...
        self.sort_col_name = tk.StringVar(value='Бюджет')
        self.col_combobox = ttk.Combobox(
            f, textvariable=self.sort_col_name, values=self.col_names, state="readonly", width=23
        )
    ...
    def build_view_tab(self, parent):
        ...
        self.view_file = tk.StringVar(value="magazines.csv")
        ...
        for i, col_id in enumerate(cols):
            self.tree.heading(col_id, text=self.col_names[i])
            self.tree.column(col_id, width=COL_WIDTHS[i], minwidth=50, stretch=False)
    ...
    def do_view(self):
        ...
        self.root.title("Магазины — Сортировка (Загрузка...)")
        ...
        def work():
            try:
                ...
                with open(fname, 'r', encoding='utf-8') as f:
                    ...
                    for row in reader:
                        if loaded >= count:
                            break
                        if len(row) > 0:
                            rows_to_show.append(_normalize_csv_row(row))
                            loaded += 1
                ...
                self.root.after(0, lambda: self.root.title("Магазины — Сортировка"))
